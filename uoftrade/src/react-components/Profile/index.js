/* Full Profile Component */
import React from "react";
import Button from 'react-bootstrap/Button'
import NavBar from "../NavBar";
import "./styles.css";
import Alert from 'react-bootstrap/Alert';
import { getProfile, updateProfile, changeProfilePicture } from "../../action/Profile"

class Profile extends React.Component {
    state = {
        updateInfo: false,
        profile_pic: {
            image_url: 'data:image/png;base64,',
            _id: -1,
            image_id: -1,
            created_at: ''
        },       
        profile: {
                bio:'',
                email:'',
                name:''
            },
        name: '',
        bio: ''   
    }


    componentDidMount() {
        getProfile(this, this.props.app.state.user)
    }

    handleInputChange = event => {
        const target = event.target;
        const value = target.value;
        const name = target.name;
        this.setState({
            [name]: value
        });
    }

    saveChanges = () => {

        if (this.state.name !== '' && this.state.bio !== '')
        {
            if (this.state.name !== this.state.profile.name || this.state.bio !== this.state.profile.bio) {
                updateProfile(this, this.props.app.state.user)
            }
        }
        
    }

    showUploadButton = () => {
        this.setState({ showUpload: !this.state.showUpload });
    }

    onFileChange = (e) => {
        changeProfilePicture(this, this.props.app.state.user, e.target)
    }

    render() {
        return (

            <div>

                <NavBar></NavBar>
                {<Alert show={this.state.updateInfo} variant='success'>
                    <p>Profile Information Updated</p>
                    <div className="d-flex justify-content-end">
                    <Button onClick={() => {this.setState({updateInfo: false})}}>
                        Close
                    </Button>
                    </div>
                </Alert>}
                <h2 className='titleProfile'> My Profile</h2>


                <div className='profileContainer'>
                    <div className='profileInfo'>
                        <h4 className='titleInfo'>Your Information</h4>

                        <div className='textBoxContainer'>
                            <div>
                                <h6>Email: </h6>
                                <input value={this.state.profile.email} readOnly name='username' />
                            </div>
                            <br />
                            <br />
                            <div>
                                <h6>Name: </h6>
                                <input
                                    placeholder={this.state.name}
                                    defaultValue={this.state.name}
                                    onChange={this.handleInputChange}
                                    name='name'
                                />
                            </div>

                            <br />
                            <div >
                                <h6>Bio </h6>
                                <textarea
                                    placeholder={this.state.bio}
                                    defaultValue={this.state.bio}
                                    onChange={this.handleInputChange}
                                    name='bio'
                                    rows={5}
                                    cols={30}
                                />
                            </div>
                        </div>

                        <div className='submitButtonContainer'>
                            <Button onClick={this.saveChanges}>Save Changes</Button>
                        </div>

                    </div>

                    <div className='profilePictureContainer'>
                        <h4 className='titleProfilePicture'>Profile Picture</h4>
                        <img className='profilePicture' src={this.state.profile_pic.image_url} alt="Profile" />
                        <div className='pictureButtonContainer'>
                            <form onSubmit={(e) => {
                                e.preventDefault();
                                this.onFileChange(e)}}>
                                
                                <input className='uploadButton' type='file' name='image' accept="image/png, image/jpeg"></input>
                                <Button type='submit' onSubmit={(e) => {
                                e.preventDefault();
                                this.onFileChange(e)}}> Upload </Button>
                            </form>
                        

                        </div>

                    </div>
                </div>
            </div>
        );
    }
}

export default Profile;